...
if ( !class_exists('phpFlickr') ) {
	...
	function throwPhpFlickrException($msg)
	{
		$flickrErrorInfo = array();
		$flickrErrorInfo['message'] = $msg;
		$flickrException = new zen\Exception\FlickrConnectionException(json_encode($flickrErrorInfo), 1);
	...
	class phpFlickr {
		...
		function post ($data, $type = null) {
			...
			if ( !preg_match("|https://(.*?)(/.*)|", $url, $matches) ) {
				throwPhpFlickrException('There was some problem figuring out your endpoint');
			}
			...
			if ( function_exists('curl_init') ) {
				...
				if (!$fp) {
					throwPhpFlickrException('Could not connect to the web service');
				}
		...
		function request ($command, $args = array(), $nocache = false)
		{
			...
			if ($this->parsed_response['stat'] == 'fail') {
				if ($this->die_on_error) {
					throwPhpFlickrException("The Flickr API returned the following error: #{$this->parsed_response['code']} - {$this->parsed_response['message']}");
				} else {
			...
			if ( function_exists('curl_init') ) {
				...
				foreach ($rsp as $line) {
					if (preg_match('|<err code="([0-9]+)" msg="(.*)"|', $line, $match)) {
						if ($this->die_on_error)
							throwPhpFlickrException("The Flickr API returned the following error: #{$match[1]} - {$match[2]}");
						else {
				...
				throwPhpFlickrException("Sorry, your server must support CURL in order to upload files");
			}
		...
		function auth ($perms = "read", $remember_uri = true, $return_url = false) {
