	...
	class phpFlickr {
		...
		function post ($data, $type = null) {
			...
			if ( !preg_match("|http://(.*?)(/.*)|", $url, $matches) ) {
				...
				$flickrErrorInfo['message'] = 'There was some problem on Flickr when trying to figure out the endpoint';
			...
			if ( function_exists('curl_init') ) {
				...
				{
					$flickrErrorInfo = array();
					$flickrErrorInfo['message'] = 'Could not connect to the Flickr web service';
				...
				{
					$flickrErrorInfo = array();
					$flickrErrorInfo['message'] = 'The web service endpoint returned a "' . $http_status . '" response';
...
//			}
			...
			if ($this->parsed_response['stat'] == 'fail') {
				...
				{
					$flickrErrorInfo = array();
					$flickrErrorInfo['message'] = "The Flickr API returned the following error: #{$this->parsed_response['code']} - {$this->parsed_response['message']}";
...
//				}
				...
				foreach ($rsp as $line) {
					if (preg_match('|<err code="([0-9]+)" msg="(.*)"|', $line, $match)) {
						...
						{
							$flickrErrorInfo = array();
							$flickrErrorInfo['message'] = "The Flickr API returned the following error: #{$match[1]} - {$match[2]}";
				...
				$flickrErrorInfo = array();
				$flickrErrorInfo['message'] = "Sorry, your server must support CURL in order to upload files on Flickr";
